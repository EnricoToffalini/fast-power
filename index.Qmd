...
- Whether there are cases in which the $N$ is difficult to estimate because the $N$-to-$StdErr$ relationship is highly noisy (as it might be the case of the SEM above);
- How to manage multiple-variable cases in which $StdErr$ does not depend on $N$ alone, but also on $k$ number of trials, $\omega$ reliability, and more; this case should be more complex but not conceptually different.
